<?php
    ...
    public function category($table){
      ...
      public function details_post_home($table_post,$post,$cond)
        {
          $sql = "SELECT * FROM $post,$table_post WHERE $cond ORDER BY $post.id_post ASC";
          return $this->db->select($sql);
        }
      public function related_post_home($table_post,$post,$cond_related)
      {
        $sql = "SELECT * FROM $post,$table_post WHERE $cond_related ORDER BY $post.id_post ASC";
        return $this->db->select($sql);
      }
      //product
      public function list_product_home($table_product){
        $sql = "SELECT * FROM $table_product ORDER BY $table_product.id_product ASC";
          return $this->db->select($sql);
      }
      public function product_hot($table_product){
        $sql = "SELECT * FROM $table_product WHERE product_hot = 1 ORDER BY $table_product.id_product ASC";
          return $this->db->select($sql);
      }
      ...
      public function product($table_product ,$table_category){
        ...
        ORDER BY $table_product.id_product ASC";
        return $this->db->select($sql);
      }
