...
<form action="<?php echo BASE_URL ?>/product/update_product/<?php echo $pro['id_product']?>" method="POST" enctype="multipart/form-data">
  ...
  <div class="form-group">
    ...
    <textarea id="editor" name="desc_product" style="resize: none;" row="5" class="form-control"> <?php echo $pro['desc_product']?></textarea>
  </div>
  <div class="form-group">
    ...
    <?php 
      ...
      ?>
    <option  <?php if($pro['id_category_product']==$cate['id_category_product']) {echo 'selected'  ;} ?> value ="<?php echo 
    $cate['id_category_product']?>"><?php echo $cate['title_category_product']?></option>
    <?php 
      }
      ?>
    </select> 
  ...
  <div class="form-group">
    <label for="pwd">Sách hot</label>
    <select class="form-control" name="product_hot" id="">
     <?php
